...
class GIF(pygame.sprite.Sprite):
    ...
    def __init__(self, path, loop, run_at_start, on_end=None, **pos):
        ...
        self.images = []
        self._image_index = -1
        Thread(target=self._load_all_images, args=[gif, run_at_start]).start()

        self.image = pygame.Surface(gif.size)
        self.rect = self.image.get_rect(**pos)

    def _load_all_images(self, gif, run_at_start):
        with load_lock:
    ...
    def update(self):
        ...
        if not self._running or now - self._last_img_time < self._secs_on_screen:
            return
